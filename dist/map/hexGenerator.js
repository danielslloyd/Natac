...
export function generateRegularHexMap(params) {
    ...
    // Build edge map
    const edgeMap = new Map(); // key -> edgeId
    ...
    const tiles = hexTiles.map(hexTile => {
...
function assignResourcesAndDice(tiles, rng) {
    ...
    if (tiles.length > shuffledResources.length) {
        ...
        const extraResources = [Resource.WOOD, Resource.BRICK, Resource.SHEEP, Resource.WHEAT, Resource.ORE];
...
export function generateStandardCatanMap(seed) {
