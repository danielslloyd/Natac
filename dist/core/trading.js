...
export function createTradeProposal(state, proposerId, proposerOffer, recipientId, recipientOffer) {
...
function validateTradeOffer(offer1, offer2, allowFutures) {
    ...
    if (!allowFutures) {
        const hasFutures = Object.keys(offer1.future).length > 0 ||
...
function getTotalResourceCount(offer) {
...
function hasResourcesForOffer(player, offer) {
...
export function agreeToTrade(state, proposalId, playerId) {
...
export function declineTrade(state, proposalId, playerId) {
    const proposal = state.tradeProposals.find(p => p.id === proposalId);
    if (!proposal) {
        return { ok: false, reason: 'Trade proposal not found' };
    }
...
export function executeTrade(state, proposalId) {
    const proposal = state.tradeProposals.find(p => p.id === proposalId);
    if (!proposal) {
    ...
    for (const [resource, amount] of Object.entries(proposal.proposerOffer.current)) {
        if (amount) {
            newProposer.resources[resource] -= amount;
            newRecipient.resources[resource] += amount;
        }
    }
    ...
    for (const [resource, amount] of Object.entries(proposal.proposerOffer.future)) {
        if (amount) {
            newProposer.futureResources[resource] =
                (newProposer.futureResources[resource] || 0) + amount;
        }
    }
    for (const [resource, amount] of Object.entries(proposal.recipientOffer.future)) {
        if (amount) {
            newRecipient.futureResources[resource] =
                (newRecipient.futureResources[resource] || 0) + amount;
        }
    }
...
export function createCounterOffer(state, originalProposalId, countererId, counterProposerOffer, counterRecipientOffer) {
    ...
    const result = createTradeProposal(state, proposerId, proposerOffer, recipientId, recipientOffer);
...
export function cleanupOldProposals(state, maxAge = 300000) {
    const now = Date.now();
    state.tradeProposals = state.tradeProposals.filter(p => now - p.createdAt < maxAge);
}
