// Bridge between mapgen.js (visualization code) and game logic
...
function convertVizDataToMapData(vizData, seed) {
  ...
  // Create tiles using graph-based boundaries
  const tiles = vizData.tiles
    .filter(tileData => tileData.isLand)
    .map(tileData => {
      const tileId = generateId('tile');
      tileIdMap.set(tileData.id, tileId);

      // Sort nodes by atan2 angle from tile center (the working winding method)
      ...
      // Get polygon points using atan2 sort from tile center
      const polygonPoints = tileData.blueNodes
        .map(vizNodeId => {
          ...
          const angle = Math.atan2(dy, dx);
          return { position: node.position, angle };
        })
        .filter(n => n !== null)
        .sort((a, b) => a.angle - b.angle)
        .map(n => n.position);
