...
(window as any).windingDebugInfo = {
  methods: [] as Array<{ tileId: string; method: WindingMethod; color: string }>
};
...
function convertVizDataToMapData(
  ...
  const tiles: Tile[] = vizData.tiles
    ...
    .map(tileData => {
      ...
      // ============================================================================
      const windingMethod: WindingMethod = Math.floor(Math.random() * 4) as WindingMethod;
      const debugColor = WINDING_DEBUG_COLORS[windingMethod];

      // Apply the selected winding method
      const polygonPoints = applyWindingMethod(
        windingMethod,
        tileData.blueNodes,
        tilePos,
        vizData
      ...
      (window as any).windingDebugInfo.methods.push({
        tileId,
        method: windingMethod,
        color: debugColor
      });
