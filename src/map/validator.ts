...
import type { Tile, Node, Edge, MapData, ValidationResult } from '../models/types.js';
...
export function validateMap(mapData: MapData): ValidationResult {
  ...
  if (tiles.length > 0) {
    ...
    while (queue.length > 0) {
      ...
      const adjacentTiles = new Set<string>();
...
export function validateMapOrThrow(mapData: MapData): void {
...
export function getAdjacentNodes(nodeId: string, edges: Edge[]): string[] {
...
export function findEdge(nodeA: string, nodeB: string, edges: Edge[]): Edge | undefined {
  return edges.find(
    e => (e.nodeA === nodeA && e.nodeB === nodeB) ||
         (e.nodeA === nodeB && e.nodeB === nodeA)
  );
