...
function generateMapData(mapType, numTiles, erosionRounds) {
    ...
    // Collect all relevant tiles (land + water) with graph-based boundaries
    const allGameTiles = new Set([...landTiles, ...waterTiles]);
    const tileDataList = [];

    for (const tileIdx of allGameTiles) {
        const trianglesForTile = tiles[tileIdx]; // Delaunay triangles containing this red point

        // Find blue nodes: centroids of triangles for this tile
        const tileBlueNodes = [];
        for (const triIdx of trianglesForTile) {
            if (validTriangles.has(triIdx)) {
                tileBlueNodes.push(triIdx);
            }
        }

        // Find green edges: Voronoi edges where BOTH triangles belong to this tile
        const tileGreenEdges = [];
        for (const [tri1, tri2] of voronoiEdges) {
            const tri1InTile = trianglesForTile.includes(tri1) && validTriangles.has(tri1);
            const tri2InTile = trianglesForTile.includes(tri2) && validTriangles.has(tri2);

            if (tri1InTile && tri2InTile) {
                tileGreenEdges.push([tri1, tri2]);
            }
        }

        tileDataList.push({
            id: tileIdx,
            position: points[tileIdx],
            blueNodes: tileBlueNodes,
            greenEdges: tileGreenEdges,
            isWater: waterTiles.has(tileIdx),
            isLand: landTiles.has(tileIdx)
        });
    }

    // Collect all unique blue nodes
    const relevantBlueNodes = new Set();
    for (const tileData of tileDataList) {
        for (const nodeIdx of tileData.blueNodes) {
            relevantBlueNodes.add(nodeIdx);
        }
    }
    ...
    // Collect unique green edges from all tiles
    const uniqueGreenEdges = new Set();
    for (const tileData of tileDataList) {
        for (const [tri1, tri2] of tileData.greenEdges) {
            const key = `${Math.min(tri1, tri2)}-${Math.max(tri1, tri2)}`;
            uniqueGreenEdges.add(key);
        }
    }
    const greenEdges = Array.from(uniqueGreenEdges).map(key => {
        const [tri1, tri2] = key.split('-').map(Number);
        return { from: tri1, to: tri2 };
    });
    ...
    return {
        tiles: tileDataList, // New format with pre-computed boundaries
        blueNodes,
        greenEdges,
        blackEdges,
        metadata: {
