 ...
 */
function isValidResourceOffer(offer: ResourceOffer, allowFutures: boolean): boolean {
  ...
  for (const resource of Object.values(ResourceEnum)) {
    if (resource === ResourceEnum.DESERT) continue; // Skip desert
  ...
  if (allowFutures && futures) {
    for (const resource of Object.values(ResourceEnum)) {
      ...
      const amount = (futures as any)[resource] || 0;
      if (amount < 0 || !Number.isInteger(amount)) {
        return false;
      }
      totalFutures += amount;
    }
  } else if (futures && Object.values(futures).some((v: any) => v > 0)) {
 ...
 */
function hasOfferResources(player: Player, offer: ResourceOffer): boolean {
  const current = offer.current;
 ...
 */
export function validateCreateTradeProposal(
  state: GameState,
  proposerId: ID,
  targetId: ID | null,
 ...
 */
export function validateAcceptTradeProposal(
  state: GameState,
  tradeId: ID,
  acceptorId: ID
): ActionResult {
 ...
 */
export function declineTradeProposal(
  state: GameState,
  tradeId: ID,
  declinerId: ID
): GameState {
 ...
 */
export function validateExecuteTrade(
  state: GameState,
  tradeId: ID
): ActionResult {
  const proposal = state.tradeProposals.find(t => t.id === tradeId);
...
export function executeTrade(
  state: GameState,
  tradeId: ID,
  acceptorId?: ID // If multiple acceptors, specify which one
): GameState {
...
export function createCounterOffer(
  ...
  const newState = createTradeProposal(
    state,
    countererId,
    originalProposal.proposerId, // Counter is directed at original proposer
    offering,
    requesting
  );
