...
export function validateAction(state: GameState, action: Action): ActionResult {
...
function validateUpgradeToCity(
  state: GameState,
  player: Player,
  nodeId: string
): ActionResult {
...
function validateBuyDevelopmentCard(
  state: GameState,
  player: Player
): ActionResult {
...
function hasResources(
  ...
  for (const [resource, amount] of Object.entries(cost)) {
    if (player.resources[resource as Resource] < amount) {
      return false;
    }
  }
  return true;
...
export function collectResources(state: GameState, diceRoll: number): GameState {
  const newState = JSON.parse(JSON.stringify(state)) as GameState;
  ...
  producingTiles.forEach(tile => {
    ...
    let tileBlockedBy: ID | null = null;
...
export function getLegalBuildLocations(state: GameState, playerId: ID): {
  settlements: string[];
  roads: string[];
} {
