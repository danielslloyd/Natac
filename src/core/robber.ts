...
import type { GameState, Player, Knight, ID } from '../models/types.js';
import { ROBBER_DISCARD_THRESHOLD } from '../models/types.js';
import { generateId, SeededRandom } from './utils.js';

export function handleRobberActivation(state: GameState): GameState {
  ...
  newState.players.forEach(player => {
    ...
    if (totalCards > ROBBER_DISCARD_THRESHOLD) {
      ...
      let discarded = 0;
      const resources = Object.keys(player.resources) as Array<keyof typeof player.resources>;
  ...
  if (stealFromPlayerId) {
    ...
    if (victim && thief) {
      ...
      if (availableResources.length > 0) {
        ...
        ] as keyof typeof victim.resources;
...
export function getPlayersOnTile(state: GameState, tileId: ID): ID[] {
