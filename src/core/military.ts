...
function deductResources(
  ...
  for (const [resource, amount] of Object.entries(cost)) {
    if (amount) {
      player.resources[resource as Resource] -= amount;
    }
  }
}
...
function getAdjacentTiles(tileId: ID, tiles: Tile[], edges: Edge[]): ID[] {
  const tile = tiles.find(t => t.id === tileId);
  if (!tile) return [];

  const adjacentTileIds = new Set<ID>();
...
export function updateKnightSupplyStatus(state: GameState): void {
...
export function purchaseMilitaryKnight(
  state: GameState,
  playerId: ID,
  tileId: ID
): ActionResult {
...
export function repositionWagon(
  state: GameState,
  playerId: ID,
  wagonId: ID,
  newEdgeId: ID
...
export function moveMilitaryKnight(
  state: GameState,
  playerId: ID,
  knightId: ID,
  targetTileId: ID
...
export function loadKnightOntoFleet(
  state: GameState,
  playerId: ID,
  knightId: ID,
  fleetId: ID
...
export function unloadKnightFromFleet(
  state: GameState,
  playerId: ID,
  fleetId: ID,
  targetTileId: ID
...
export function updateCaptureProgress(state: GameState): void {
...
function captureSettlement(state: GameState, nodeId: ID, attackerId: ID): void {
  const node = state.nodes.find(n => n.id === nodeId);
...
export function collectMaintenanceCosts(state: GameState, playerId: ID): ActionResult {
  if (!state.options.militaryMode) {
    return { ok: true };
  }
  ...
  const totalCost: Record<Resource, number> = {
    [Resource.ORE]: 0,
    [Resource.SHEEP]: 0,
    [Resource.WOOD]: 0,
    [Resource.BRICK]: 0,
...
export function resetMovementFlags(state: GameState): void {
  if (!state.options.militaryMode) return;
