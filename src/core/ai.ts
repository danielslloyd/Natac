...
const RESOURCE_VALUES: Record<AIPersonality, Record<Resource, number>> = {
  robert: {
...
export function evaluateTradeOffer(
  ...
  if (!player.aiPersonality) return 0;
  ...
  for (const [resource, amount] of Object.entries(giving.current)) {
    if (amount) {
      givingValue += values[resource as Resource] * amount;
    }
  }
  for (const [resource, amount] of Object.entries(giving.future)) {
    if (amount) {
      givingValue += values[resource as Resource] * amount * 0.7; // Future resources worth less
    }
  }
  ...
  for (const [resource, amount] of Object.entries(receiving.current)) {
    if (amount) {
      receivingValue += values[resource as Resource] * amount;
    }
  }
  for (const [resource, amount] of Object.entries(receiving.future)) {
    if (amount) {
      receivingValue += values[resource as Resource] * amount * 0.7;
    }
  }
...
export function aiShouldAcceptTrade(
  ...
  if (!player.isAI || !player.aiPersonality) return false;
...
export function aiGenerateTradeProposal(
  state: GameState,
  aiPlayer: Player,
  rng: SeededRandom
): TradeProposal | null {
  if (!aiPlayer.isAI || !aiPlayer.aiPersonality) return null;
  ...
  const wants: Resource[] = [];
  const hasExcess: Resource[] = [];
  ...
  if (personality === 'robert') {
    if (aiPlayer.resources[ResourceEnum.WOOD] < 3) wants.push(ResourceEnum.WOOD);
    if (aiPlayer.resources[ResourceEnum.BRICK] < 3) wants.push(ResourceEnum.BRICK);
  }

  // Lenore wants ore/wheat for cities
  if (personality === 'lenore') {
    if (aiPlayer.resources[ResourceEnum.ORE] < 5) wants.push(ResourceEnum.ORE);
    if (aiPlayer.resources[ResourceEnum.WHEAT] < 3) wants.push(ResourceEnum.WHEAT);
  }
...
export function aiMakeCounterOffer(
  state: GameState,
  aiPlayer: Player,
  ...
  const counterAiOffer: TradeOffer = {
    current: {},
    future: {}
  };

  const counterOtherOffer: TradeOffer = {
    current: {},
    future: {}
  };
  ...
  for (const [resource, amount] of Object.entries(originalAiOffer.current)) {
    if (amount) {
      // Offer less
      counterAiOffer.current[resource as Resource] = Math.max(1, Math.floor(amount * 0.7));
    }
  }
  ...
  for (const [resource, amount] of Object.entries(originalOtherOffer.current)) {
    if (amount) {
      // Ask for more
      counterOtherOffer.current[resource as Resource] = Math.ceil(amount * 1.3);
    }
  }
  ...
  if (totalAi === 0 || totalOther === 0) return null;
...
export function aiDecideBuildAction(
  player: Player,
  state: GameState
): 'road' | 'settlement' | 'city' | null {
  if (!player.isAI || !player.aiPersonality) return null;
